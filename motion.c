...
extern volatile uint8
    next_motion_page;      // next motion page if we got new command
...
// the new implementation of AVR libc does not allow variables passed to
// _delay_ms
...
void motionPageInit() {
  ...
  for (int i = 0; i < MAX_AX12_SERVOS; i++) {
    if (AX12_ENABLED[i] != AX12Servos[i]) {
      ...
      printf("\nConfiguration of enabled AX-12 servos does not match motion.h. "
             "ABORT!\n");
      exit(-1);
    }
  }
...
// It implements a finite state machine to know what it is doing and what to do
// next
// Code is meant to be reentrant so it can easily be converted to a task with a
// RTOS
void executeMotionSequence() {
  ...
  // Walk State = %i, Current Step = %i", motion_state, walkGetWalkState(),
  // current_step);

  // check the states in order of likelihood of occurrence
  // the most likely state is that a motion step is still being executed or
  // paused
  if (motion_state == STEP_IN_MOTION) {
    ...
    if (walk_getWalkState() != 0) {
      if ((millis() - step_start_time) >=
          CurrentMotion.PlayTime[current_step - 1]) {
    ...
    if ((millis() - pause_start_time) >=
        CurrentMotion.PauseTime[current_step - 1]) {
  ...
  // Next we check for any movement related alarms - at this point the only way
  // the
  ...
  //				2. Respond to a change in walk command (see WALK EXECUTE
  //task in RoboPlus Task sample files)
  //				3. Start Pause Time after step
  //				4. Execute next step in current motion page
  //				5. Go back to first step in current motion page
  //(repeat)
  ...
  if (current_step == CurrentMotion.Steps &&
      (motion_state == STEP_FINISHED || motion_state == PAUSE_FINISHED)) {
    // check if we just finished an exit page
    if (exit_flag == 1) {
      // yes, reset flag and change motion state and then return to not
      // complicate things
      exit_flag = 0;
      motion_state = MOTION_STOPPED;
      return;
    ...
    // Option 5 - repeat the current motion page
      ...
      // can go straight to executing step 1 since we have executed this page
      // before
      step_start_time = executeMotionStep(current_step);
      return;
    }
    // Option 6 - switch to NextPage motion page
    else if (CurrentMotion.NextPage > 0 &&
             CurrentMotion.NextPage <= NUM_MOTION_PAGES) {
  ...
  if (motion_state == STEP_FINISHED) {
    ...
    if (CurrentMotion.PauseTime[current_step - 1] > 0 &&
  ...
  if (motion_state == MOTION_STOPPED && new_command == TRUE) {
    // special case for walk commands we need to get walk ready if we weren't
    // walking before
    if ((last_bioloid_command == COMMAND_STOP ||
         last_bioloid_command > COMMAND_WALK_READY) &&
        (bioloid_command >= COMMAND_WALK_FORWARD &&
         bioloid_command < COMMAND_WALK_READY)) {
      // this is the only time we wait for a motion to finish before returning
      // to the command loop!
      walk_init();
    }
    // special case of shifting between walk commands - non-seamless transitions
    else if (walk_getWalkState() > 0 &&
             (bioloid_command >= COMMAND_WALK_FORWARD &&
              bioloid_command < COMMAND_WALK_READY)) {
      ...
      next_motion_page = (bioloid_command - 1) * 12 + COMMAND_WALK_READY_MP +
                         left_right_step + 1;
    }
...
void unpackMotion(int StartPage) {
  ...
  CurrentMotion.NextPage =
      pgm_read_byte(motion_pointer[StartPage] + NUM_AX12_SERVOS + 0);
  CurrentMotion.ExitPage =
      pgm_read_byte(motion_pointer[StartPage] + NUM_AX12_SERVOS + 1);
  CurrentMotion.RepeatTime =
      pgm_read_byte(motion_pointer[StartPage] + NUM_AX12_SERVOS + 2);
  CurrentMotion.SpeedRate10 =
      pgm_read_byte(motion_pointer[StartPage] + NUM_AX12_SERVOS + 3);
  CurrentMotion.InertialForce =
      pgm_read_byte(motion_pointer[StartPage] + NUM_AX12_SERVOS + 4);
  CurrentMotion.Steps =
      pgm_read_byte(motion_pointer[StartPage] + NUM_AX12_SERVOS + 5);
  ...
  for (s = 0; s < CurrentMotion.Steps; s++) {
    for (i = 0; i < num_packed_steps; i++) {
      // higher 16bit
      packed_step_values =
          pgm_read_word(motion_pointer[StartPage] + NUM_AX12_SERVOS + 6 +
                        (s * 4 * num_packed_steps) + 4 * i + 2);
      packed_step_values = packed_step_values << 16;
      // lower 16bit
      packed_step_values +=
          pgm_read_word(motion_pointer[StartPage] + NUM_AX12_SERVOS + 6 +
                        (s * 4 * num_packed_steps) + 4 * i);
      // unpack and store
    ...
    // Sanity Check - if the values are outside the overall Min/Max values we
    // are probably accessing random memory
    for (i = 0; i < NUM_AX12_SERVOS; i++) {
      if (CurrentMotion.StepValues[s][i] > SERVO_MAX_VALUES[i] ||
        ...
        printf("\nUnpack Motion Page %i, Step %i - rubbish data. STOP.",
               StartPage, s + 1);
        printf("\nServo ID%i, Step Value = %i, Min = %i, Max = %i \n",
               AX12_IDS[i], CurrentMotion.StepValues[s][i], SERVO_MIN_VALUES[i],
               SERVO_MAX_VALUES[i]);
        exit(-1);
      }
    }
  ...
  for (s = 0; s < CurrentMotion.Steps; s++) {
    CurrentMotion.PauseTime[s] = pgm_read_word(
        motion_pointer[StartPage] +
        (NUM_AX12_SERVOS + 6 + CurrentMotion.Steps * 4 * num_packed_steps) +
        (s * 2));
    if (CurrentMotion.PauseTime[s] != 0 && CurrentMotion.PauseTime[s] < 6500) {
      CurrentMotion.PauseTime[s] =
          (10 * CurrentMotion.PauseTime[s]) / CurrentMotion.SpeedRate10;
    } else {
      CurrentMotion.PauseTime[s] =
          10 * (CurrentMotion.PauseTime[s] / CurrentMotion.SpeedRate10);
    }
  ...
  for (s = 0; s < CurrentMotion.Steps; s++) {
    CurrentMotion.PlayTime[s] = pgm_read_word(
        motion_pointer[StartPage] +
        (NUM_AX12_SERVOS + 6 + CurrentMotion.Steps * 4 * num_packed_steps +
         CurrentMotion.Steps * 2) +
        (s * 2));
    if (CurrentMotion.PlayTime[s] != 0 && CurrentMotion.PlayTime[s] < 6500) {
      CurrentMotion.PlayTime[s] =
          (10 * CurrentMotion.PlayTime[s]) / CurrentMotion.SpeedRate10;
    } else {
      CurrentMotion.PlayTime[s] =
          10 * (CurrentMotion.PlayTime[s] / CurrentMotion.SpeedRate10);
    }
...
// This function initiates the execution of a motion step in the given motion
// page
...
unsigned long executeMotionStep(int Step) {
  ...
  if (Step > 0 && Step <= CurrentMotion.Steps) {
    ...
    moveToGoalPose(CurrentMotion.PlayTime[Step - 1], goalPose,
                   DONT_WAIT_FOR_POSE_FINISH);
...
int setMotionPageJointFlexibility() {
  ...
  for (uint8 i = 0; i < NUM_AX12_SERVOS; i++) {
    ...
    if (last_joint_flex[i] != CurrentMotion.JointFlex[i]) {
      ...
      commStatus = dxl_write_byte(AX12_IDS[i], DXL_CCW_COMPLIANCE_SLOPE,
                                  complianceSlope);
...
int executeMotion(int StartPage) {
  ...
  // TEST: for (int s=0; s<CurrentMotion.Steps; s++) { printf(" %lu,",
  // step_times[s]); }
